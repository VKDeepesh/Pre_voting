...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        self.mqtt_client = paho.Client("control1")  # Create MQTT client object
        self.mqtt_client.username_pw_set(ACCESS_TOKEN)  # Access token from thingsboard device
        self.mqtt_client.connect(broker, port, keepalive=60)  # Establish MQTT connection
    ...
    def process_face(self, frame, face_location):
        ...
        if matches[best_match_index]:
            ...
            if self.last_face_detected_time is None or name != self.last_face_name:
                ...
                # Publish person name and reward points to MQTT
                self.publish_to_mqtt(name, self.reward_points[name])
    ...
    def publish_to_mqtt(self, name, points):
        payload = {
            "Person_Name": name,
